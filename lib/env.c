...
u_int mkenvid(struct Env *e) {
	...
	// asid low 11 bits: idx, i.e. ENVX(env_id)
}
...
env_alloc(struct Env **new, u_int parent_id)
{
	...
	*new = NULL;
	...
	if ((r = env_setup_vm(e)) == -E_NO_MEM) return r;
	...
	e->env_tf.regs[29] = USTACKTOP;  // regs[29]: sp
...
env_create_priority(u_char *binary, int size, int priority)
{
	...
	LIST_INSERT_HEAD(env_sched_list, e, env_sched_link); // env_sched_list i.e. &env_sched_list[0]
}
/* Overview:
