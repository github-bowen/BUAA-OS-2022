...
int page_insert(Pde *pgdir, struct Page *pp, u_long va, u_int perm)
{
	...
	if (pgtable_entry != 0 && (*pgtable_entry & PTE_V) != 0) {  //pgtable_entry has created before
		// check whether `va` is mapping to another physical frame
		if (pa2page(*pgtable_entry) != pp) { // mapping to another physical frame
	...
	}  // pgtable_entry hasn't created before
	/* Step 2: Update TLB. */
